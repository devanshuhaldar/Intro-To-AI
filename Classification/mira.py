...
class MiraClassifier:
    ...
    def update_weights(self, trainingData, trainingLabels, C):
        for iteration in range(self.max_iterations):
            ...
            for i in range(len(trainingData)):
                ...
                if guess != trainingLabels[i]:
                    ...
                    datum_scale = {feature: value * tau for feature, value in datum.items()}
                    self.weights[guess] -= datum_scale
                    self.weights[trainingLabels[i]] += datum_scale

    def calculate_accuracy(self, validationData, validationLabels):
        guesses = self.classify(validationData)
        accuracy = sum(1 for guess, actual in zip(guesses, validationLabels) if guess == actual)
        return accuracy
    ...
    def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, Cgrid):
        ...
        for C in Cgrid:
            ...
            self.update_weights(trainingData, trainingLabels, C)
            accuracy = self.calculate_accuracy(validationData, validationLabels)
